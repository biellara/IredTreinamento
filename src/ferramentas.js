document.addEventListener("DOMContentLoaded", function () {
  ...
  function showTool(targetId) {
    ...
    if (!template) {
      console.error(
        `Template para a ferramenta "${targetId}" não foi encontrado no HTML.`
      );
      return;
    }
    ...
    analyzeBtn.addEventListener("click", async () => {
      ...
      try {
        ...
        document.getElementById(
          "diagnosis-output"
        ).innerHTML = `<strong>Erro ao contactar a IA:</strong> ${error.message}`;
        aiResults.style.display = "block";
      } finally {
  ...
  function setupSimulador() {
    ...
    const scenarios = {
      ...
      "wifi-nao-funciona-leigo":
        "Cliente leigo com Wi-Fi que não funciona no quarto.",
...
`;
      ...
      try {
        ...
        try {
          ...
          const cleanedText = feedbackResponseText
            .replace(/```json/g, "")
            .replace(/```/g, "")
            .trim();
          ...
          console.error(
            "Falha ao fazer parse do JSON da IA. Usando texto completo.",
            e
          );
        ...
        const feedbackText =
          ...
          "Não foi possível gerar o feedback em texto.";
        const feedbackScores = feedbackData.scores || {
          empathy: 0,
          clarity: 0,
          resolution: 0,
        };
        ...
        if (!response.ok) {
          ...
          throw new Error(
            errorData.error ||
              `Erro ao salvar simulação: ${response.statusText}`
          );
        }
        ...
        showError(
          error.message ||
            "Houve um problema ao finalizar a simulação. Tente novamente."
        );
      } finally {
        simLoader.style.display = "none";
        isSimulating = false;
  ...
  function setupRelatorio() {
    ...
    generateReportBtn.addEventListener("click", async () => {
      ...
      const prompt = `Formate o seguinte resumo como um relatório técnico de atendimento, utilizando obrigatoriamente as três seções abaixo:

                1. Relato do Cliente – Descreva de forma objetiva o que foi informado pelo cliente no início do atendimento, incluindo sintomas, reclamações ou solicitações.

                2. Procedimentos Realizados – Detalhe todas as ações executadas pelo atendente, como diagnósticos, testes, verificações, ajustes ou tentativas de solução aplicadas.

                3. Conclusão – Indique o desfecho do atendimento, especificando se o problema foi resolvido, se houve necessidade de encaminhamento, agendamento ou qualquer outro encaminhamento.

                Utilize uma linguagem clara, técnica e profissional. Evite repetições, informalidades e generalizações.
    ...
    copyReportBtn.addEventListener("click", () => {
      ...
      const copyBtnTextContainer = document.getElementById(
        "copyBtnTextContainer"
      );
      const originalText = copyBtnTextContainer.innerHTML;
      copyBtnTextContainer.innerHTML = "<span>Copiado!</span>";
      setTimeout(() => {
        copyBtnTextContainer.innerHTML = originalText;
      }, 2000);
    });
  }
  ...
  async function callGeminiAPI(promptOrHistory) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(
          `A chamada para a API falhou: ${response.status} - ${errorBody}`
        );
      }
