...
function abrirAssistente() {
  const modal = document.getElementById("modalAssistente");
 ...
 * @param {'user' | 'model' | 'loading'} sender - Quem enviou a mensagem.
 */
function adicionarMensagemAoChat(text, sender) {
  const chatContainer = document.getElementById("chatContainer");
  if (!chatContainer) return;

  // Remove a mensagem de "carregando..." se houver uma
  ...
  // --- CORREÇÃO PRINCIPAL ---
  // Se a mensagem for do modelo, converte Markdown para HTML para renderizar formatação.
  // Se for do usuário, continua usando textContent por segurança.
  if (sender === 'model') {
    const html = converter.makeHtml(text); // Usa o conversor Showdown
...
document.addEventListener("DOMContentLoaded", () => {
  const enviarBtn = document.getElementById("btnEnviarAssistente");
  const input = document.getElementById("mensagemAssistente");
  ...
  if (enviarBtn && input) {
    const lidarComEnvio = async () => {
      ...
      adicionarMensagemAoChat("...", 'loading');
      ...
      try {
        ...
        const textoResposta = dados?.candidates?.[0]?.content?.parts?.[0]?.text || "Não foi possível obter uma resposta.";
        
        conversationHistory.push({ role: 'model', parts: [{ text: textoResposta }] });
        adicionarMensagemAoChat(textoResposta, 'model');

      } catch (err) {
        console.error("[ASSISTENTE] Erro:", err);
        adicionarMensagemAoChat("Ocorreu um erro ao consultar o assistente. Tente novamente.", 'model');
      } finally {
