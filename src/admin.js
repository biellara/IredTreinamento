$(document).ready(function () {
  ...
  async function loadSimulationsDashboard() {
    try {
      ...
      if (!response.ok)
        throw new Error("Não foi possível carregar o dashboard de simulações.");
      ...
      $.getScript("dashboard.js", function () {
        if (typeof setupSimulationsDashboard === "function") {
          ...
          console.error(
            "ERRO CRÍTICO: A função 'setupSimulationsDashboard' não foi encontrada em dashboard.js."
          );
        }
      });
    } catch (error) {
      ...
      $("#view-simulations").html(
        '<p style="color: red;">Ocorreu um erro ao carregar esta secção.</p>'
      );
    }
  }
  ...
  async function loadQuizzesManagement() {
    $.getScript("criarQuiz.js", function () {
      if (typeof setupQuizzesManagement === "function") {
        ...
        console.error(
          "ERRO CRÍTICO: A função 'setupQuizzesManagement' não foi encontrada em criarQuiz.js."
        );
      }
    });
  }
  ...
  async function loadDashboardStats() {
    try {
      ...
      if (usersResponse.ok)
        $("#stats-total-users").text((await usersResponse.json()).length);
      if (simsResponse.ok) {
      ...
      if (quizzesResponse.ok)
        $("#stats-total-quizzes").text((await quizzesResponse.json()).length);
    } catch (error) {
