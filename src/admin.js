$(document).ready(function () {
  ...
  async function loadAndRenderQuizData() {
    try {
      ...
      if (!resultsResponse.ok)
        throw new Error("Falha ao carregar resultados dos quizzes.");

      const results = await resultsResponse.json();
      const quizzes =
        allQuizzes.length > 0
          ? allQuizzes
          : await (await secureFetch("/api/getQuiz")).json();
      ...
      $("#quizPerformanceChart")
        .parent()
        .html(
          '<p style="color: red;">Não foi possível carregar o gráfico de desempenho.</p>'
        );
      $("#quizResultsTable")
        .parent()
        .html(
          '<p style="color: red;">Não foi possível carregar a tabela de resultados.</p>'
        );
    }
  }
  ...
  async function setupSimulationsView() {
    try {
      ...
      if (simulations.length === 0) {
        $("#view-simulations .content-card").html(
          '<p class="text-center text-gray-500 p-10">Nenhuma simulação encontrada.</p>'
        );
        return;
      }
        ...
        columns: [
          ...
          },
          {
            ...
            render: (data, type, row) =>
              `<button class="btn-action btn-feedback" data-id="${row.id}" title="Ver Feedback"><i class="fas fa-comment-dots"></i></button> <button class="btn-action btn-history" data-id="${row.id}" title="Ver Histórico"><i class="fas fa-history"></i></button>`,
          },
        ],
      ...
      $("#view-simulations").html(
        '<p class="text-center text-red-500 p-10">Não foi possível carregar os dados.</p>'
      );
    }
  }
  ...
  function populateUserFilter(simulations) {
    ...
    if (userFilter.length) {
      ...
      const users = [
        ...new Set(simulations.map((s) => s.username).filter(Boolean)),
      ];
      users
        .sort()
        .forEach((user) =>
    ...
    container
      ...
      .on("click", ".btn-feedback", function () {
        ...
        if (simulation) {
          ...
          if (
            typeof simulation.feedback === "string" &&
            !simulation.feedback.startsWith("[")
          ) {
    ...
    container
      ...
      .on("click", ".btn-history", function () {
        ...
        if (simulation) {
          ...
          const conversationData = Array.isArray(simulation.chatHistory)
          ...
          if (conversationData.length > 0) {
            conversationData.forEach((msg) => {
              ...
              historyHtml += `<p><strong>${speaker}:</strong> ${formatText(
                text
              )}</p>`;
            });
          } else {
  ...
  function setupExportButtons(simulation) {
    ...
    ).on("click", () => exportToPdf(simulation));
    const btnCsv = $(
      '<button class="btn btn-secondary">Exportar CSV</button>'
    ).on("click", () => exportToCsv(simulation));
  ...
  function setupExportHistoryButtons(simulation) {
    ...
    const btnPdf = $(
      '<button class="btn btn-secondary">Exportar PDF</button>'
    ).on("click", () => exportHistoryToPdf(simulation));
    const btnCsv = $(
      '<button class="btn btn-secondary">Exportar CSV</button>'
    ).on("click", () => exportHistoryToCsv(simulation));
  ...
  function exportHistoryToPdf(simulation) {
    ...
    y += lineHeight;
    doc.setFontSize(12);
    const chat = Array.isArray(simulation.chatHistory)
      ? simulation.chatHistory
      : [];
    chat.forEach((msg) => {
      if (msg.role !== "user" && msg.role !== "model") return;
  ...
  async function loadQuizzesManagement() {
    try {
      ...
      quizzesTable = $("#quizzesTable").DataTable({
        ...
        columns: [
          ...
          },
          {
            ...
            orderable: false,
            render: (data, type, row) =>
              `<button class="btn-action edit-btn" data-id="${row.id}" title="Editar"><i class="fas fa-edit"></i></button> <button class="btn-action delete-btn" data-id="${row.id}" title="Excluir"><i class="fas fa-trash"></i></button>`,
          },
        ],
        language: dataTableLanguage,
      });
      ...
      $("#view-quizzes .content-card-header").after(
        '<p style="color:red;">Não foi possível carregar os quizzes.</p>'
      );
    }
  }
  ...
  function addQuestionHTML(question = {}) {
    ...
    let optionsHTML = options
      ...
      <div class="option-entry">
        <!-- Radio button com valor atrelado dinamicamente -->
        <input 
          type="radio" 
      ...
      usersTable = $("#usersTable").DataTable({
        ...
        columns: [
          ...
          { data: "role", title: "Função" },
          {
            ...
            orderable: false,
            render: (data) =>
              `<button class="btn-action edit-btn" data-id="${data}" title="Editar"><i class="fas fa-edit"></i></button> <button class="btn-action delete-btn" data-id="${data}" title="Excluir"><i class="fas fa-trash"></i></button>`,
          },
        ],
        language: dataTableLanguage,
      });
  ...
  $("#quizzesTable").on("click", ".delete-btn", function () {
    ...
    $("#deleteModal p").text(
      "Tem a certeza de que deseja excluir este quiz? Esta ação não pode ser revertida."
    );
  ...
  $("#quizForm").on("submit", async function (e) {
    ...
    const questions = [];
    $(".question-block").each(function (index, el) {
      ...
      if (questionText && options.length >= 2 && answer) {
        ...
        console.warn("Pergunta inválida detectada!");
      }
    });
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          (await response.json()).error || "Falha ao salvar o quiz."
        );
  ...
  $("#questions-container").on("click", ".btn-add-option", function () {
    ...
    const newOption = `
          <div class="option-entry">
              ...
              <input type="text" class="form-input option-input" placeholder="Opção ${
                optionIndex + 1
              }" value="" required>
              <button type="button" class="btn-remove-option">&times;</button>
          </div>`;
  ...
  $("#cancelQuizBtn, #quizModal .modal-close-btn").on("click", () =>
    $("#quizModal").removeClass("active")
  );

  $("#addUserBtn").on("click", () => openUserModal());
  $("#usersTable").on("click", ".edit-btn", function () {
    openUserModal(usersTable.row($(this).parents("tr")).data());
  });
  $("#usersTable").on("click", ".delete-btn", function () {
    ...
    $("#deleteModal p").text(
      "Tem a certeza de que deseja excluir este colaborador? Esta ação não pode ser revertida."
    );
    $("#deleteModal").addClass("active");
  });
  ...
  $("#userForm").on("submit", async function (e) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          (await response.json()).error || "Falha ao salvar utilizador."
        );
  ...
  $("#articlesTable").on("click", ".edit-btn", function () {
    openArticleModal(articlesTable.row($(this).parents("tr")).data());
  });
  $("#articlesTable").on("click", ".delete-btn", function () {
    ...
    $("#deleteModal p").text(
      "Tem a certeza de que deseja excluir este artigo? Esta ação não pode ser revertida."
    );
    $("#deleteModal").addClass("active");
  });
  ...
  $("#articleForm").on("submit", async function (e) {
    ...
    try {
      ...
      if (!response.ok)
        throw new Error(
          (await response.json()).error || "Falha ao salvar o artigo."
        );
  ...
  $("#articleModal .modal-close-btn").on("click", () =>
    $("#articleModal").removeClass("active")
  );
  $("#cancelDeleteBtn, #deleteModal .modal-close-btn").on("click", () =>
    $("#deleteModal").removeClass("active")
  );
